...
def _n_cluster(dataset, k=1, alpha=1, max_iter=100, tol=10e-2, relevance=None):
    ...
    for idx, n in enumerate(range(max_iter)):
        ...
        sampler.fit(dataset, sample_weight=np.maximum(0, relevance))
...
def kmeans_sampler(
    ...
    dist = (pairwise_distances(clusters, dataset) * relevance).sum(axis=0)
...
class FreddyTrainer(SubsetTrainer):
    ...
    def _select_subset(self, epoch, training_step):
        ...
        score = self.train_criterion(
            torch.from_numpy(feat), torch.from_numpy(tgt)
        ).detach().numpy() * (
            (-(p1 * np.log2(1 + p1))).sum() / (-(p2 * np.log2(1 + p2))).sum()
        )
